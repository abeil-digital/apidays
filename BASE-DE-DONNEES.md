...
| Table                    | Rôle                                                                                                                                                                                                                                                                                            |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `entreprises`             | Un tenant client (09/09/2026, chantier multi-tenant) — nom, slug, branding, `date_debut_utilisation` ("epoch" du tenant). Presque toutes les autres tables portent un `entreprise_id` qui y réfère. Détail complet (schéma, RLS, onboarding) dans [MULTI-TENANT.md](MULTI-TENANT.md), pas dupliqué ici. |
...
| `demandes_conges`        | Les demandes elles-mêmes — dates, statut, décision, dévalidation. `conge_impose_id` (29/08/2026, nullable) marque les lignes générées automatiquement par un CPI — voir `conges_imposes` ci-dessous                                                                                             |
...
| `conges_imposes`         | Périodes de congés imposés (ex. semaine du 15 août) pour une période donnée — source de vérité pour la PÉRIODE (Paramétrer > Calendrier) ; depuis le 29/08/2026, déduit le solde CP comme un CP normal via des lignes `demandes_conges` générées (`conge_impose_id`), plus indépendant du solde |
...
| `soldes_periode`         | Gel du capital d'ouverture CP à la bascule (11/09/2026, refonte du moteur de calcul) — une ligne par utilisateur/période une fois celle-ci close, écrite paresseusement au premier calcul rencontré après sa fin ; jamais recalculée ensuite, y compris si les règles changent après coup      |
| `acquisitions_gelees`    | Gel de l'acquisition mensuelle RTT/CPA (11/09/2026) — une ligne par utilisateur/type/mois, écrite à la validation ("pris en compte") de l'export paie qui couvre ce mois ; protège l'accrual déjà confirmé en paie d'un changement ultérieur du taux d'acquisition (voir `exports_paie` ci-dessous) |
| `exports_paie`           | Transmission paie (Suivre > Transmissions paie, 24/08/2026) — un enregistrement par clic sur "Transmettre", une seule transmission par période (`exports_paie_periode_unique`) ; "Valider" (11/09/2026) marque `pris_en_compte`, fige la consommation ET déclenche `acquisitions_gelees` ci-dessus |
...
- **Congés imposés (CPI), mêmes règles de gestion qu'un CP normal** (29/08/2026, revirement par
  rapport au comportement d'origine — indépendant du solde jusque-là, voir historique CONTEXTE.md) :
  `conges_imposes` reste la source de vérité pour la PÉRIODE (ce que Delphine crée/consulte/supprime
  sur Paramétrer > Calendrier), mais sa création génère désormais une vraie ligne `demandes_conges`
  (type CP, statut `validee`) **par collaborateur actif**, liée via `demandes_conges.conge_impose_id`
  (FK nullable, `on delete set null`). Choix délibéré plutôt que de dupliquer le calcul de solde/
  export paie pour un second type d'enregistrement : ces lignes traversent alors tout le pipeline
  existant sans aucun changement de code (`soldes.repository.ts`, `exportsPaie.repository.ts` ne
  filtrent ni sur l'origine ni sur l'utilisateur — un CP `validee` de n'importe qui est ramassé tel
  quel), donc solde Théorique déduit dès la création, solde Réel déduit une fois transmis en paie,
  exactement comme un CP posé par le salarié. Un collaborateur ne peut **pas** annuler lui-même une
  telle ligne (policy RLS "demandes: salarié annule un congé validé non transmis" exclut
  `conge_impose_id is not null`) — seul l'admin, en supprimant la période sur Paramétrer > Calendrier
  (`supprimerCongeImpose`), qui annule alors (jamais un hard delete, `retirerDemande`) chaque demande
  liée encore validée. Limitation connue, assumée : un collaborateur arrivant après la création d'un
  CPI n'a pas de demande générée rétroactivement (contournement : supprimer puis recréer la
  période).
