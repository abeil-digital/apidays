...
function LigneFormulaireFaq({ valeurInitiale, onValider, onAnnuler }: LigneFormulaireFaqProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 px-4 py-4">
      <div>
        <label htmlFor="faq-question" className="text-ink-900 mb-1.5 block text-sm font-bold">
          Question
        </label>
        <Input
          id="faq-question"
      ...
      <div>
        <label htmlFor="faq-reponse" className="text-ink-900 mb-1.5 block text-sm font-bold">
          Réponse
        </label>
        <Textarea
          id="faq-reponse"
...
export function FaqAdminPage() {
  ...
  return (
    <div className="flex w-full max-w-md flex-col gap-5 pt-5 pb-4 md:max-w-2xl md:pt-0">
      <h1 className="text-ink-900 animate-stagger-in px-1 text-2xl font-semibold">FAQ</h1>
      ...
      {loading ? (
        ...
        <div className="bg-surface-card border-ink-300/60 flex flex-col gap-5 border p-5">
          <div>
            <h2 className="text-ink-900 text-sm font-bold">Questions fréquentes</h2>
