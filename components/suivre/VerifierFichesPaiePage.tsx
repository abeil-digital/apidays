 ...
 */
export function VerifierFichesPaiePage({ exportId }: { exportId: string | null }) {
          ...
          return (
            <div key={collab.utilisateur.id} className="bg-surface-card w-full shadow-sm">
              <div className="flex flex-wrap items-center gap-3 px-4 py-3">
                ...
                <button
                  type="button"
                  onClick={() => setOuvert((v) => (v === collab.utilisateur.id ? null : collab.utilisateur.id))}
                  className="text-ink-500 flex items-center gap-1 text-xs font-semibold"
                >
                  Détail
                  {ouvert === collab.utilisateur.id ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                </button>
              </div>
              ...
              {ouvert === collab.utilisateur.id && (
                <div className="border-ink-300/60 border-t">
                  {collab.lignes.map(({ ligne, demande }) => (
                    <div key={ligne.id} className="border-ink-300/60 flex flex-col gap-2 border-b px-4 py-3 last:border-b-0">
