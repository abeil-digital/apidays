...
export async function getSiteUrl(): Promise<string> {
  ...
  // Diagnostic temporaire (08/09/2026) — le lien email pointait vers
  // localhost même déclenché depuis la prod, cause non confirmée. À
  // retirer une fois le comportement réel observé dans les logs Vercel.
  console.log("[getSiteUrl] headers", {
    xForwardedHost: h.get("x-forwarded-host"),
    host: h.get("host"),
    xForwardedProto: h.get("x-forwarded-proto"),
    resultat: `${protocole}://${host}`,
  });
